...
export class UserRelationDAO {
  ...
  async getFollowing(userId: string): Promise<UserRelation[]> {
    const { rows } = await pool.query<UserRelation>(
      'SELECT * FROM user_relations WHERE follower_id = $1 AND status = \'accepted\' ORDER BY created_at DESC',
      [userId]
    );
    logger.info(`[UserRelationBddRepository] Getting following users for user ${userId}`);
    return rows;
  }
