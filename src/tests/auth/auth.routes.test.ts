// Tests for OAuth routes (Google)
// We mock passport for the callback flow and verify the redirect URL for the initial step.
import request from 'supertest';

// Mock passport BEFORE importing the app so that the routes use the mocked implementation
jest.mock('../../config/passport', () => {
    const mockPassport = {
        ...
        // passport.initialize() middleware no-op for tests
...
describe('Auth Routes (Google OAuth)', () => {
    ...
    it('GET /auth/google should redirect to Google with scope and client_id', async () => {
        ...
        // URLSearchParams encodes spaces as + for application/x-www-form-urlencoded
