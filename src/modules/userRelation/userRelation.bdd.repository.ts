...
import { UserRelation } from './userRelation.types';
...
export class UserRelationBddRepository {
    ...
    async unfollow(follow : FollowDTO): Promise<void> {
        await pool.query(
            'DELETE FROM user_relation WHERE followed_id = $1 AND follower_id = $2',
            [follow.followed_id, follow.follower_id]
        );
    ...
    async accept(follow : FollowDTO): Promise<void> {
        await pool.query(
            'UPDATE user_relation SET status = \'accepted\' WHERE followed_id = $1 AND follower_id = $2',
            [follow.followed_id, follow.follower_id]
        );
        await pool.query(
            'INSERT INTO user_relation (followed_id, follower_id, status) VALUES ($1, $2, \'accepted\') ON CONFLICT (followed_id, follower_id) DO NOTHING',
    ...
    async refuse(follow : FollowDTO): Promise<void> {
        await pool.query(
            'UPDATE user_relation SET status = \'refused\' WHERE followed_id = $1 AND follower_id = $2',
            [follow.followed_id, follow.follower_id]
        );
    ...
    async block(follow : FollowDTO): Promise<void> {
        await pool.query(
            `INSERT INTO user_relation (followed_id, follower_id, status) 
            VALUES ($1, $2, 'blocked') 
    ...
    async getFriends(userId: string): Promise<UserRelation[]> {
        const { rows } = await pool.query<UserRelation>(
            'SELECT * FROM user_relation WHERE (followed_id = $1 ) AND status = \'accepted\'',
            [userId]
        );
    ...
    async getPendingRequests(userId: string): Promise<UserRelation[]> {
        const { rows } = await pool.query<UserRelation>(
            'SELECT * FROM user_relation WHERE followed_id = $1 AND status = \'pending\'',
            [userId]
        );
    ...
    async findRelation(follow : FollowDTO): Promise<UserRelation | null> {
        const { rows } = await pool.query<UserRelation>(
            'SELECT * FROM user_relation WHERE followed_id = $1 AND follower_id = $2',
            [follow.followed_id, follow.follower_id]
        );
